...
const SearchPage = () => {
  ...
  //  you're sending the query parameters (searchQuery, categories, and sortByPrice) to the backend via your RTK Query API slice.

  const isEmpty = !isLoading && data?.courses.length === 0;

  // we get categories & price  from filter.jsx, we pass this fn as Props in Filter.jsx
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row gap-10">
        ...
        <div className="flex-1">
          {/* if is Loading we show 3 skelton , say data is loading */}
          {isLoading ? (
            Array.from({ length: 3 }).map((_, idx) => (
              <CourseSkeleton key={idx} />
            ))
          ) : isEmpty ? (  //if we have no Course, like we have a emptyresult, we show this Elemt 
            <CourseNotFound />
          ) : (  //otherwise we display the course 
